<div wire:ignore.self id="createUserModal" modal-center
    class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show">
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
            <h5 class="text-16">Add New User</h5>
            <button data-modal-close="createUserModal" wire:ignore
                class="transition-all duration-200 ease-linear text-slate-400 hover:text-slate-500"><i data-lucide="x"
                    class="size-5"></i></button>
        </div>
        <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
            <form class="tablelist-form" wire:submit='saveUser'>
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="email" id="email-field" wire:model='email'
                        class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                ...
                @if (Auth::user()->role == 0)
                    <div class="mb-3">
                        <label for="role-field" class="inline-block mb-2 text-base font-medium">Role <span
                                class="text-red-500">*</span></label>
                        <select wire:model='role'
                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                            data-trigger name="role-field" id="status-field">
                            @foreach ($roleName as $key => $role)
                                <option wire:key='{{ $key }}' value="{{ $key }}">{{ $role }}
                                </option>
                            @endforeach
                        </select>
                    </div>
