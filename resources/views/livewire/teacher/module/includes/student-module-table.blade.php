@if ($usersModule->isNotEmpty())
    <table class="w-full whitespace-nowrap">
        ...
        <tbody>
            @foreach ($usersModule as $userModule)
                <tr>
                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                        <a href="#!">{{ isset($userModule->identity) ? $userModule->identity : '-' }}</a>
                    </td>
                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                        <div class="flex gap-2">
                            <div class="bg-green-100 rounded-full size-10 dark:bg-green-500/20 shrink-0">
                                <img src="{{ $userModule->profile_photo_url }}" alt=""
                                    class="h-10 rounded-full">
                            </div>
                            <div class="grow">
                                <h6>{{ $userModule->name }}</h6>
                                <p class="text-slate-500 dark:text-zink-200">{{ $userModule->email }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                        <div class="flex gap-2">
                            <button wire:click='removeUserFromClass({{ $userModule->id }})'
